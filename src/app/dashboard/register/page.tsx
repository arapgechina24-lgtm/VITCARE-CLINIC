...
export default function RegisterPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!user) return;
      const { data } = await supabase.from('user_site_memberships').select('site_id').eq('user_id', user.id).limit(1).maybeSingle();
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!siteId) return setError("Your account isn't assigned to a site.");
    setBusy(true);
    setError(null);
